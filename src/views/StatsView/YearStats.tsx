  ...
  return (
    <div>
      ...
      <div className="glass p-4 mb-4">
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', height: 150, paddingBottom: 20, borderBottom: '1px solid var(--border-color)', position: 'relative' }}>
          {monthTotals.map((amount, i) => (
            <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '6%' }}>
              {amount > 0 && <div style={{ fontSize: '0.6rem', color: 'var(--text-secondary)', marginBottom: 2, writingMode: 'vertical-rl', textOrientation: 'mixed', transform: 'rotate(180deg)' }}>{amount / 1000}k</div>}
              ...
              <div style={{ position: 'absolute', bottom: -20, fontSize: '0.7rem', color: 'var(--text-secondary)' }}>{i + 1}</div>
            </div>
          ))}
        </div>
      </div>
